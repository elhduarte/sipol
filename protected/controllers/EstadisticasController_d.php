...
public function actionCorreoenviotabla()
{
	...
	if (empty($tipo_sede)|| $tipo_sede =="00") {
		$condicion="";
		$tiposede="";
	}else
	{
	...
	if(empty($sede)|| $sede =="00"){
		$condicion2="";
		$sede="";
	} else
	{			
		$condicion2=" and ts.id_sede = ".$sedes;
	}

function llamarhoy($id_hecho,$condicion,$condicion2,$comisaria)
{
	$querydos ="SELECT count(ed.id_evento) as conteo
	FROM sipol.tbl_evento_detalle ed
	INNER JOIN sipol.tbl_evento e ON ed.id_evento = e.id_evento
	...
	and ed.atributos IS NOT NULL
	and ts.id_cat_entidad = ".$comisaria."
	and ts.id_tipo_sede = ".$tiposede."
	and ts.id_sede = ".$sedes."
	AND cd.id_cat_denuncia = ".$id_hecho."
	GROUP BY cd,id_cat_denuncia";
	$resultadodos = Yii::app()->db->createCommand($querydos)->queryScalar();
	$salida = 0;
...
function llamarayer($id_hecho,$condicion,$condicion2,$comisaria)
{
	$querydos ="SELECT count(ed.id_evento) as conteo
	FROM sipol.tbl_evento_detalle ed
	INNER JOIN sipol.tbl_evento e ON ed.id_evento = e.id_evento
...
function llamarunasemana($id_hecho,$condicion,$condicion2,$comisaria)
{
	$querydos ="SELECT count(ed.id_evento) as conteo
	FROM sipol.tbl_evento_detalle ed
	INNER JOIN sipol.tbl_evento e ON ed.id_evento = e.id_evento
...
function llamardossemana($id_hecho,$tiposede,$sedes,$comisaria)
{
	$querydos ="SELECT count(ed.id_evento) as conteo
	FROM sipol.tbl_evento_detalle ed
	INNER JOIN sipol.tbl_evento e ON ed.id_evento = e.id_evento
...
function llamarunmes($id_hecho,$tiposede,$sedes,$comisaria)
{
	$querydos ="SELECT count(ed.id_evento) as conteo
	FROM sipol.tbl_evento_detalle ed
	INNER JOIN sipol.tbl_evento e ON ed.id_evento = e.id_evento
...
function procesafila($nombrehecho,$id_hecho,$condicion,$condicion2,$comisaria,$contador)
{
	$hoy = llamarhoy($id_hecho,$condicion,$condicion2,$comisaria);
	$ayer = llamarayer($id_hecho,$condicion,$condicion2,$comisaria);
	$unasaemana = llamarunasemana($id_hecho,$condicion,$condicion2,$comisaria);
	$dossemanas = llamardossemana($id_hecho,$condicion,$condicion2,$comisaria);
	$unmes = llamarunmes($id_hecho,$tiposede,$sedes,$comisaria);
	//echo $nombrehecho."-".$hoy."-".$ayer."-".$unasaemana."-".$unmes;
	//echo "<br>";
  ...
  <thead>
    <tr>
      ...
      <th>Comisaria</th>
       ...
       <th><center>Ayer</center></th>
        <th></th>
       <th><center>1 Semana</center></th>
        <th><center>2 Semanas</center></th>
       <th></th>
       <th><center>1 Mes</center></th>
    </tr>
  </thead>
  <tbody>';
       $query = "SELECT id_cat_denuncia, nombre_denuncia  FROM sipol_catalogos.cat_denuncia order by nombre_denuncia";
...
$contador = 0;
foreach ($resultado as $key => $value) {
	$tabla .= procesafila($value['nombre_denuncia'],$value['id_cat_denuncia'],$tiposede,$sedes,$comisaria,$contador);
	$contador = $contador +1;
}
 $tabla .='</tbody></table>';
//echo $tabla;
...
function llamardossemana($id_hecho,$condicion,$condicion2,$comisaria)
{
	$querydos ="SELECT count(ed.id_evento) as conteo
	FROM sipol.tbl_evento_detalle ed
	INNER JOIN sipol.tbl_evento e ON ed.id_evento = e.id_evento
...
function llamarunmes($id_hecho,$condicion,$condicion2,$comisaria)
{
	$querydos ="SELECT count(ed.id_evento) as conteo
	FROM sipol.tbl_evento_detalle ed
	INNER JOIN sipol.tbl_evento e ON ed.id_evento = e.id_evento
...
function procesafila($nombrehecho,$id_hecho,$condicion,$condicion2,$comisaria,$contador)
{
	...
	$unasaemana = llamarunasemana($id_hecho,$condicion,$condicion2,$comisaria);
	$dossemanas = llamardossemana($id_hecho,$condicion,$condicion2,$comisaria);
	$unmes = llamarunmes($id_hecho,$condicion,$condicion2,$comisaria);
	$mesanterior = llamarmesanterior($id_hecho,$comisaria);
	//echo $nombrehecho."-".$hoy."-".$ayer."-".$unasaemana."-".$unmes;
	//echo "<br>";
$restaanailizhoy = 0;
...
$contador = 0;
foreach ($resultado as $key => $value) {
	$tabla .= procesafila($value['nombre_denuncia'],$value['id_cat_denuncia'],$tiposede,$sede,$comisaria,$contador);
	$contador = $contador +1;
}
 $tabla .='</tbody></table>';
echo '	<div id="exportarBotones" align="right"> 
		...
		<a href="index.php?r=Estadisticas/mostrarexcelfiltros&nuevo='.$comisaria.'&tipoevento='.$tipoevento.'&tiposede='.$tiposede.'&sedes='.$sede.'"  role="button" class="btn btn-large btn-success" data-toggle="modal">EXCEL</a>  
	</div><br>'.$tabla;
}
...
$tabla = '<table class="table table-bordered">
  <thead>
   ...
   <td>'.$value['count'].'</td>
    <td><a href="index.php?r=Estadisticas/Mapassipol&geo='.$encriptar->compilarget(trim($tipoevento)).','.$encriptar->compilarget(trim($tiposede)).','.$encriptar->compilarget(trim($sedes)).','.$encriptar->compilarget(trim($tiempo)).','.$encriptar->compilarget(trim($comisaria)).','.$encriptar->compilarget(trim($value['id_cat_denuncia'])).'"  class="btn btn-mini btn-primary" target="_blank" >Analizar Hecho</a></td>
    </tr>';
