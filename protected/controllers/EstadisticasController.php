...
public function actiontablacomisaria()
{
	$fechainicio = $_POST['fechainicio'];
	$fechafin = $_POST['fechafin'];
	$comisaria = $_POST['comisaria'];
	$select="select id_cat_entidad from catalogos_publicos.cat_entidad where nombre_entidad='".$comisaria."'";
	$resultadodos = Yii::app()->db->createCommand($select)->queryAll();
		 foreach ($resultadodos as  $value) {
		 	  $comisaria2= $value['id_cat_entidad'];
		 }
	...
	if ((empty($fechainicio)) || (empty($fechafin))) {
		...
		$fechafin = "'". $fechafin . "'";
		$condicion.=" and ed.fecha_evento between ".$fechainicio." and ".$fechafin;
	}
	if (empty($comisaria)) {
		$condicion.="";
	}
	...
	{
		$condicion.="and en.id_cat_entidad = ".$comisaria2;
	}

	$query = "SELECT en.id_cat_entidad, en.nombre_entidad entidad,ch.id_cat_denuncia id_denuncia, ch.nombre_denuncia, count(ch.*) 
FROM
       sipol.tbl_evento e, catalogos_publicos.tbl_sede s, 
       catalogos_publicos.cat_entidad en, 
       catalogos_publicos.cat_tipo_sede ts, 
       sipol.tbl_evento_detalle ed,
       sipol_catalogos.cat_denuncia ch
where 
       en.id_cat_entidad = s.id_cat_entidad
       and ts.id_tipo_sede = s.id_tipo_sede
       and e.id_sede = s.id_sede
       and ed.id_evento = e.id_evento
       and ed.atributos is not null
       and e.id_tipo_evento=1 ".$condicion."
       and e.estado = 't' 
       and ch.id_cat_denuncia = ed.id_hecho_denuncia
group by 
       en.id_cat_entidad, en.nombre_entidad,ch.nombre_denuncia, ch.id_cat_denuncia  order by en.id_cat_entidad";

	$resultado = Yii::app()->db->createCommand($query)->queryAll();

	$url2 =Yii::app()->createUrl('Estadisticas/pnivelcomisaria');

	$tabla="
    <script type='text/javascript'>
  	$('.ttip_hecho').tooltip({title:'Click en el Nombre Hecho Para Ver Detalle'});
	$('.id_hecho2').click(function(e){
		e.stopImmediatePropagation();
		var idhecho = $(this).data('idhecho');
		var identidad = $(this).data('identidad');
		var fecha_inicio = $('#fechaIniGral4').val();
		var fecha_fin = $('#fechaFinGral4').val();
	  $.ajax({
        type:'POST',
        url:'$url2',
          data:{
...
$tabla .= ' <br> <br> <legend>'.$comisaria.'</legend> 
<table class="table table-striped" id="tabla_segundo_nivel">
  <thead>
    <tr>
      <th>Hecho</th>
      <th>Comisaria</th>
  ...
  <tbody>';
	...
	$contador = $contador +1;
	$id_hecho= $value ['id_denuncia'];
	$nombrehecho=$value['nombre_denuncia'];
	$id_entidad=$value['id_cat_entidad'];
	$nombre_entidad=$value['entidad'];
	$count=$value['count'];
	$tabla.="  <tr>
	
	 <td><div id='idhecho2' class='id_hecho2'  data-identidad=".$id_entidad." data-idhecho =".$id_hecho.">".$nombrehecho."</div></td>
...
if( $unmes > $mesanterior)
	...
	$url =Yii::app()->createUrl('Estadisticas/pnivel');
	$salidagrafica ="
    <script type='text/javascript'>
...
echo '	<div id="exportarBotones" align="right"> 
		...
		<a href="index.php?r=Estadisticas/mostrarexcelfiltros&nuevo='.$comisaria.'&tipoevento='.$tipoevento.'&tiposede='.$tiposede.'&sedes='.$sedes.'"  role="button" class="btn btn-large btn-success" data-toggle="modal">EXCEL</a>  
	</div><br>'.$tabla;
}
...
echo '	<div id="exportarBotones" align="right"> 
		...
		<a href="index.php?r=Estadisticas/mostrarexcel&nuevo='.$comisaria.'&tipoevento='.$tipoevento.'"  role="button" class="btn btn-large btn-success" data-toggle="modal">EXCEL</a>  
	</div><br>'.$tabla;
}
...
$tabla = '<table class="table table-bordered">
  <thead>
   ...
   <td>'.$value['count'].'</td>
    <td>
    <a href="index.php?r=Estadisticas/Mapassipol&geo='.$encriptar->compilarget(trim($tipoevento)).','.$encriptar->compilarget(trim($tiposede)).','.$encriptar->compilarget(trim($sedes)).','.$encriptar->compilarget(trim($tiempo)).','.$encriptar->compilarget(trim($comisaria)).','.$encriptar->compilarget(trim($value['id_cat_denuncia'])).','.$encriptar->compilarget(trim($bandera)).'"  class="btn btn-mini btn-primary" target="_blank" >Analizar Hecho</a></td>
						...
						$tabla = '<table class="table table-bordered">
						  <thead>
						    <tr>
						      ...
						      <td>'.$value['nombre_denuncia'].'</td>
						    <td>'.$value['count'].'</td>
						    <td><a href="index.php?r=Estadisticas/Mapassipol&geo='.$encriptar->compilarget(trim($tipoevento)).','.$encriptar->compilarget(trim($tiposede)).','.$encriptar->compilarget(trim($sedes)).','.$encriptar->compilarget(trim($tiempo)).','.$encriptar->compilarget(trim($comisaria)).','.$encriptar->compilarget(trim($value['id_cat_denuncia'])).','.$encriptar->compilarget(trim($bandera)).'"  class="btn btn-mini btn-primary" target="_blank" >Ver Mapa</a></td>
						    </tr>';
    ...
    <tr>
      ...
      <th></th>
		...
		foreach ($resultado as $key => $value) 
		{
			...
			<td>'.$value['nombre_denuncia'].'</td>
			<td>'.$value['count'].'</td>
			<td><a href="index.php?r=Estadisticas/Mapassipol&geo='.$encriptar->compilarget(trim($tipoevento)).','.$encriptar->compilarget(trim($tiposede)).','.$encriptar->compilarget(trim($sedes)).','.$encriptar->compilarget(trim($tiempo)).','.$encriptar->compilarget(trim($comisaria)).','.$encriptar->compilarget(trim($value['id_cat_denuncia'])).','.$encriptar->compilarget(trim($bandera)).','.$encriptar->compilarget(trim($fechauno)).','.$encriptar->compilarget(trim($fechados)).'"  class="btn btn-mini btn-primary" target="_blank" >Ver Mapa</a></td>
			</tr>';
...
public function actionErrorNovedades()
	{
		...
		public function actionPnivelcomisaria()
	{
		$this->renderPartial('primer_nivel_comisaria');
	}
public function actionSnivelcomisaria()
	{
		$this->renderPartial('segundo_nivel_comisaria');
	}
