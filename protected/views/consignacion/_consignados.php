...
<?php 
	...
	$form = $this->beginWidget('bootstrap.widgets.TbActiveForm', array(
		...
		//'type'=>'vertical',
		'method' => 'post',
		'action' => Yii::app()->createUrl('Consignacion/ProcesaHechos'),
...
<div style="display:none;" id="hechos_resumen_well">
	...
	<legend>Objetos y/o Personas consignados:</legend>
...
<script type="text/javascript">
	...
	$('#FrmConsignados').submit(function(e){
		...
		if(idEvento == 'empty'){
			$.ajaxq('funcionuno',{
				type:'POST',
				timeout:30000,
				url:'<?php echo Yii::app()->createUrl("Consignacion/Save_incidencia"); ?>',
				data:{
					idEvento:idEvento
				},
				...
				success:function(response){
					$('#identificador_consignacion').val(response);
					idEvento = response;
					saveConsignado(idEvento);
				}
	...
	saveConsignado = function(idEvento)
	{
		...
		$.ajaxq('funcionuno',{
			type: 'POST',
			timeout:30000,
			url: '<?php echo Yii::app()->createUrl("Consignacion/Save_consignados"); ?>',
			data: 
			{
			...
			success: function(respuesta)
			{
				...
				$('#cli_agentes').attr('class','enabled');
	...
	} // Fin de la función SaveConsignado
