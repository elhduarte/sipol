...
if($identificacionTipo =='Sin Documento')
{
	...
	$ide = ", QUIEN NO PRESENTÓ DOCUMENTO DE IDENTIFICACIÓN";
		...
		$bodyresponsables = strtoupper(htmlentities("SE LE EXTIENDE LA PRESENTE CONSTANCIA EN UNA HOJA DE PAPEL BOND, MEMBRETADA, SELLADA Y FIRMADA EL DÍA  ".$funciones->fechaATexto($encabezado['fecha_ingreso'], 'u')."; ASIGNADA CON EL CODIGO UNICO DE DENUNCIA ".$encabezado['evento_num']." (".$variabledenuncialetras.")."));
		...
		#$bodydenunciante = strtoupper(utf8_decode("".$denunciante['Nombre_Completo'].", DE ".$denunciante['Edad']." AÑOS, GÉNERO ".$denunciante['Genero'].", ESTADO CIVIL ".$denunciante['Estado_Civil'].$profesion.", NACIDO EN EL PAÍS ".$denunciante['Pais_de_Nacimiento'].$departamento.$municipio.", CON DOMICILIO ACTUAL ".$denunciante['Direccion_de_contacto'].$ide.$identificacionTipo.$numeroIdentificacion.$identificacionLetras.$nombrePadre.$nombreMadre."."));
		#$bodyresponsables = strtoupper(utf8_decode("PARA EL USO QUE AL INTERESADO(A) CONVENGA, SE LE EXTIENDE LA PRESENTE CONSTANCIA EN UNA HOJA DE PAPEL BOND, MEMBRETADA, SELLADA Y FIRMADA EL DÍA  ".$funciones->fechaATexto($encabezado['fecha_ingreso'], 'u')."; ASIGNADA CON EL CODIGO UNICO DE DENUNCIA ".$encabezado['evento_num']." (".$variabledenuncialetras.")."));
		#$ubicacion = strtoupper(utf8_decode('DENUNCIANDO QUE EN LA UBICACIÓN SIGUIENTE: '.$ubicacion."."));
		...
		function Header()
		{
			  ...
			  // $this->Image($_SESSION['qr'],180,5,25);
			   ...
			   $this->Ln(15);
				...
				$this->RotatedText(35,190,'Generado desde Estadistica',45);
		}
...
$pdf->Ln(12);
$pdf->SetFont('courier','B',12);
$pdf->MultiCell(0, 5,utf8_decode('EXTENSIÓN DE DENUNCIA '), 0, "C", 0);
$pdf->Ln(5);
$pdf->SetFont('courier','',10);
$pdf->MultiCell(0, 5,utf8_decode(strtoupper('SE REALIZÓ UNA EXTENSIÓN DE ÉSTA DENUNCIA, EL DÍA: '.$funciones->fechaATexto($fecha,'u').' A LAS:'.$funciones->horaALetras($hora))), 0, "J", 0);
$pdf->Ln(5);
$pdf->SetFont('courier','B',10);
$pdf->MultiCell(0, 5,'OCURRIERON LOS HECHOS SIGUIENTES', 0, "C", 0);
$pdf->Ln(3);
...
$pdf->Ln(5);
$pdf->Output('temp/denuncias/'.$encabezado['evento_num'].'.pdf','F');
echo  $encabezado['evento_num'].".pdf";
