...
//$geoserver = 'http://geoserver.mingob.gob.gt/geoserver/';
?>
<div class="well">
  ...
  $.fn.actualizaMupio = function(recibe,mupio,band)
  {
    ...
    var idMupio = $('#cmb_mupio ').val();
      ...
      geo_save[8]=idMupio;
			...
			$.ajax({
				type:'POST',
				url: '<?php echo CController::createUrl("Incidencia/SaveUbicacion"); ?>',
				data:
				{
				...
				success: function(response)
				{
					...
					$('#ili_relato').attr('class','enabled');
					$('#i_relato').removeClass('BotonClose');                     
					$('#tab3').hide(50);
					$('#tab4').show(50);
					$('#nav_incidencia li:eq(3) a').tab('show');
				}
			});
		}
	}); //fin submit #geo
