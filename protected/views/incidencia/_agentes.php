...
$(document).ready(function(){
	...
	$('#ayuda_agente').tooltip({html:true,title:'<legend class="legend_tt"><i class="icon-question-sign icon-white"></i> AYUDA</legend>'+
		'<p style="line-height: 175%; text-align: justify;">Ingrese el NIP del agente para realizar la búsqueda de éste y relacionarlo con ésta incidencia.</p>'});
	$('#ayuda_patrulla').tooltip({html:true,title:'<legend class="legend_tt"><i class="icon-question-sign icon-white"></i> AYUDA</legend>'+
		'<p style="line-height: 175%; text-align: justify;">Ingrese los datos de la patrulla para relacionarla con ésta incidencia.</p>'});
	...
	$('#frmBuscaAgente').submit(function(e){
		...
		$.ajax({
			type:'POST',
			url:"<?php echo Yii::app()->createUrl('Incidencia/Procesa_agente'); ?>",
			data:{nip:nip},
			beforeSend:function()
			{
	...
	$('#frmBuscaPatrulla').submit(function(e){
		...
		$.ajax({
			type:'POST',
			url:"<?php echo Yii::app()->createUrl('Incidencia/Procesa_patrulla'); ?>",
	...
	$.fn.MostrarAgentes = function(valor)
	{
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo Yii::app()->createUrl("Incidencia/ShowAgentes"); ?>',
			data: {idEvento:valor,},
			success: function(divs)
			{
				...
				$(".DeleteAgente").click(function(){
					...
					$('#CommitDeleteAgente').click(function(){
						...
						$.ajax({
						type:'POST',
						url:'<?php echo Yii::app()->createUrl("Incidencia/DeleteAgente"); ?>',
				...
				$(".DeletePatrulla").click(function(){
					...
					$('#CommitDeletePatrulla').click(function(){
						...
						$.ajax({
						type:'POST',
						url:'<?php echo Yii::app()->createUrl("Incidencia/DeletePatrulla"); ?>',
						data:{id_eliminar: id_eliminar,},
						beforeSend: function()
						{
							$('#result_procesoModal').html('');
