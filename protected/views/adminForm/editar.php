...
<div class="cuerpo">
	<?php 
	$TblSede = new TblSede;
	$getSede =  $TblSede->getSede($el_id);

	?>
	<legend><?php echo "Editar Sede: ".$getSede['nombre'] .",En ".$getSede['departamento'].", ".$getSede['municipio'];?> </legend>
	...
	<form id="addCatalogo">
		<div class="row-fluid">
            ...
            ?>
					...
					<button class="btn btn-primary span12" type="button" style="margin-top: 9%;" id="savesede">Editar Sede</button>
					</div>
...
$('#savesede').click(function(){     
  var entidad = $('#entidad').val();
    ...
    var id_sede = '<?php echo $el_id = $_GET['id']?>';
    

   $.ajax({
          type: "POST",
          url: <?php echo "'".CController::createUrl('AdminForm/UpdateSede')."'"; ?>,
          data: {
          	entidad: entidad,
          	tipo_sede: tipo_sede,
          	depto: depto,
         ...
         //alert('ya va');
          ...
          success: function(result)
          {
           window.location.replace('<?php echo CController::createUrl("adminForm/admin"); ?>');
          }
        });
