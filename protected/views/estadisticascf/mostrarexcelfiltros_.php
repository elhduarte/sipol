...
function llamarhoy($id_hecho,$comisaria,$condicion)
{
	...
	where e.fecha_ingreso  BETWEEN 'now'::text::date AND 'now'::text::date
	and e.estado = 't'
...
function llamarayer($id_hecho,$comisaria,$condicion)
{
	$querydos ="SELECT count(ed.id_evento) as conteo
	FROM sipol.tbl_evento_detalle ed
	INNER JOIN sipol.tbl_evento e ON ed.id_evento = e.id_evento
	INNER JOIN sipol_catalogos.cat_denuncia cd ON ed.id_hecho_denuncia = cd.id_cat_denuncia
	INNER JOIN catalogos_publicos.tbl_sede ts ON e.id_sede = ts.id_sede
	where e.fecha_ingreso  BETWEEN 'now'::text::date -1 AND 'now'::text::date -1
	and e.estado = 't'
	and e.id_tipo_evento = 1
	and ed.atributos IS NOT NULL
		and ts.id_cat_entidad = ".$comisaria." 	 ".$condicion."
...
function llamarunasemana($id_hecho,$comisaria,$condicion)
{
	$querydos ="SELECT count(ed.id_evento) as conteo
	FROM sipol.tbl_evento_detalle ed
	INNER JOIN sipol.tbl_evento e ON ed.id_evento = e.id_evento
	INNER JOIN sipol_catalogos.cat_denuncia cd ON ed.id_hecho_denuncia = cd.id_cat_denuncia
	INNER JOIN catalogos_publicos.tbl_sede ts ON e.id_sede = ts.id_sede
	where e.fecha_ingreso  BETWEEN 'now'::text::date -7 AND 'now'::text::date
	and e.estado = 't'
	and e.id_tipo_evento = 1
	and ed.atributos IS NOT NULL
		and ts.id_cat_entidad = ".$comisaria."	 ".$condicion."
...
function llamardossemana($id_hecho,$comisaria,$condicion)
{
	$querydos ="SELECT count(ed.id_evento) as conteo
	FROM sipol.tbl_evento_detalle ed
	INNER JOIN sipol.tbl_evento e ON ed.id_evento = e.id_evento
	INNER JOIN sipol_catalogos.cat_denuncia cd ON ed.id_hecho_denuncia = cd.id_cat_denuncia
	INNER JOIN catalogos_publicos.tbl_sede ts ON e.id_sede = ts.id_sede
	where e.fecha_ingreso  BETWEEN 'now'::text::date -14 AND 'now'::text::date -7
	and e.estado = 't'
	and e.id_tipo_evento = 1
	and ed.atributos IS NOT NULL
		and ts.id_cat_entidad = ".$comisaria."	 ".$condicion."
...
function llamarunmes($id_hecho,$comisaria,$condicion)
{
	$querydos ="SELECT count(ed.id_evento) as conteo
	FROM sipol.tbl_evento_detalle ed
	INNER JOIN sipol.tbl_evento e ON ed.id_evento = e.id_evento
	INNER JOIN sipol_catalogos.cat_denuncia cd ON ed.id_hecho_denuncia = cd.id_cat_denuncia
	INNER JOIN catalogos_publicos.tbl_sede ts ON e.id_sede = ts.id_sede
	where e.fecha_ingreso  BETWEEN 'now'::text::date -30 AND 'now'::text::date
	and e.estado = 't'
	and e.id_tipo_evento = 1
	and ed.atributos IS NOT NULL
		and ts.id_cat_entidad = ".$comisaria."	 ".$condicion."
...
function llamarmesanterior($id_hecho,$comisaria,$condicion)
{
	$querydos ="SELECT count(ed.id_evento) as conteo
	FROM sipol.tbl_evento_detalle ed
	INNER JOIN sipol.tbl_evento e ON ed.id_evento = e.id_evento
...
function procesafila($nombrehecho,$id_hecho,$comisaria,$contador,$objPHPExcel,$tiposede,$sedes,$condicion)
{
	$hoy = llamarhoy($id_hecho,$comisaria,$condicion);
	$ayer = llamarayer($id_hecho,$comisaria,$condicion);
	$unasaemana = llamarunasemana($id_hecho,$comisaria,$condicion);
	$dossemanas = llamardossemana($id_hecho,$comisaria,$condicion);
	$unmes = llamarunmes($id_hecho,$comisaria,$condicion);
	$mesanterior = llamarmesanterior($id_hecho,$comisaria,$condicion);
