...
if(isset($_POST['tipoRender']))
{
	switch ($_POST['tipoRender']) 
	{
		...
		case 'llave':
			...
			$llave = $_POST['llave'];
			$resultadoLlave = $WSConsulta->ConsultaLlave($llave);		
			$dpiLlave = $resultadoLlave['dpi'];	
			$resultado = $WSConsulta->ConsultaDpiCaptura($dpiLlave);
			if(isset($resultado['error']))$error = $resultado['error'];
			
			if(isset($resultado['solvente']))$solvente = $resultado['solvente'];
		...
		case 'dpi':
			...
			$dpi = $_POST['dpi'];
			$resultado = $WSConsulta->ConsultaDpiCaptura($dpi);
			$llave="";
			if(isset($resultado['solvente']))$solvente = $resultado['solvente'];
			if(isset($resultado['error']))$error = $resultado['error'];
